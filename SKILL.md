...
**Default — Water Environment deck.** If the user does not supply an explicit template directory path, automatically dispatch the built-in deck template at:

```text
${SKILL_DIR}/templates/decks/water-environment
```

Do NOT ask which template to use. Do NOT query other `*_index.json` files unless the user asks what templates exist.
...
| Anything else — bare template names ("用 academic_defense"), style descriptions ("麦肯锡风格"), brand mentions ("招商银行风格"), vague intent ("想用个模板"), or silence | Dispatch the default Water Environment deck |
...
**Upstream methodology pass (Mandatory — not a ninth confirmation)**: after the Eight Confirmations are approved and before writing `design_spec.md` / `spec_lock.md`, Strategist MUST distill the source material into a planning brief:

1. Write `<project_path>/source_digest.md` for file/URL/source-heavy projects. For short chat-only briefs, embed the same summary in `design_spec.md §IX Source Digest Summary`.
2. Convert raw source into a structured extraction table before page planning: entities, dimensions, metrics, time periods, pain points, recommendations, evidence, and source locations where applicable.
3. Record density assessment: source type, approximate information load, recommended page count, and the rule used to avoid text dumping (usually one core assertion plus 3-5 supporting facts per content slide; `entities × dimensions ÷ 3` is a useful rough sizing check for comparison-heavy decks).
4. Record audience and narrative plan: map the locked `mode` to the deck's argument arc; if a classic framework helps (Pyramid, SCQA, STAR, AIDA, hero journey), name it as supporting rationale without replacing the official `mode` value.
5. Ensure every §IX page plan carries or implies: title, core message, purpose, layout, content texture, visualization/image intent, and so-what. Use the official `Core message` field as the spine; add `Purpose` / `So What` when the implication is not obvious.
6. Build a title track: read slide titles in sequence and confirm the narrative can be reconstructed from titles alone. Weak title track means the outline needs restructuring before SVG generation.
7. Decide the reusable deck chrome (footer, page number, accent line, section marker, title treatment) and the per-page insight strip / takeaway treatment so inner pages feel like one deck instead of independent posters.

**Output**:
- `<project_path>/source_digest.md` — source digestion and density/narrative planning brief (for source-heavy projects)
...
- [x] Source digest, density assessment, and narrative plan completed
