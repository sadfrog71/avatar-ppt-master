...
**Per-page brief extraction**: Before drawing each page, restate the current §IX entry to yourself as a compact execution brief:
...
- Insight strip / takeaway line, plus reusable `chrome` from `spec_lock.md` when present

If `Core message` is missing or the inferred so-what is unclear, do not paper over it with decoration. Surface the ambiguity and revise the outline before generating that page.

**Information translation defaults**: Choose the visual form from the source shape, not from habit. Single critical number → hero metric; 3-5 parallel items → card grid; 5-8 ordered steps → timeline / stepped flow; 9+ items → categorize or split; comparison data → table with color coding; process / evolution → timeline or arrows; conceptual relationships → diagram. If the page cannot fit its selected form cleanly, revise the §IX plan instead of shrinking text until it breaks.

**Chrome consistency**: Reuse deck furniture from `spec_lock.md chrome` and the cover / first inner page: footer treatment, page numbering, section marker, accent line, title placement, and insight strip style. Template mirror mode can override geometry, but free-design pages should still feel like members of the same deck.
