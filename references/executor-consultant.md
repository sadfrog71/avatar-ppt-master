...
### 5. Expanded Chart Type Specification (v1.2)

For Analysis-type decks, the following 10 chart types are available. All charts MUST use native SVG construction (rect, line, circle, path, text, tspan) or reference templates from `templates/charts/`.

| Chart Type | Best Scenario | SVG Construction Notes | Anti-patterns |
| ---------- | ------------- | ---------------------- | ------------- |
| **Column Clustered** | Multi-entity same-dimension comparison | Bar width uniform, gap = 50% bar width, data labels at top | 3D columns, rainbow colors, missing labels |
| **Bar Stacked** | Composition + comparison | Max 5 stack categories, theme-color order, total label on right | Too many stacks, no total, unreadable labels |
| **Pie / Doughnut** | Single-entity composition | Max 5-7 slices, clockwise by size, percentage + name labels | >7 slices, 3D pie, missing labels |
| **Radar** | Multi-dimension capability assessment | 3-6 dimensions, fill-opacity 20-30%, max 3 comparison series | >8 dimensions, opaque fill, no axis labels |
| **Line** | Time-series trends | 2-3pt stroke width, visible markers, max 3 trend lines | >3 lines, no markers, missing data points |
| **Waterfall** | Increment/decrement breakdown | Up=success color, down=warning color, total=primary color | Missing total bar, inconsistent colors |
| **Funnel** | Process conversion stages | 3-5 stages, decreasing width, conversion rate labels | >5 stages, no rate labels, asymmetric widths |
| **Scatter / Bubble** | Distribution/correlation analysis | Trend line optional, bubble limit 20, radius proportional to 3rd metric | Too many bubbles, radius not scaled |
| **Heatmap** | Matrix density / correlation | Gradient from light (surface) to dark (primary), value labels in cells | Rainbow gradient, missing labels |
| **Combo (Column + Line)** | Absolute value + rate dual-metric | Column=surface fill + primary stroke, line=accent, single Y-axis | Dual Y-axes (FORBIDDEN in consulting decks) |

**Chart Mandatory Elements** (every chart MUST include):
1. **Title**: 13-16px, primary color, positioned above chart area
2. **Data Labels**: Values displayed directly on/above bars, points, or slices (skip only when density makes unreadable)
3. **Axis Labels**: Categories + values with units
4. **Legend**: Bottom or right-aligned, 11-12px
5. **Source Caption**: "资料来源: ...", 10px, secondary text color, bottom-left of chart area

**Chart Color Series Order**:
```
Series 1: primary (100% opacity)
Series 2: accent (100% opacity)
Series 3: secondary_accent (100% opacity)
Series 4: badge_info (#3182CE)
Series 5: badge_warning (#F6AD55)
Series 6: badge_leading (#38A169)
```

**Grid lines**: border color (#E2E8F0 equivalent), 0.5pt stroke, horizontal only (line charts may have vertical reference lines).
