...
Turn source material into an argument, not a decorated summary. Before page planning, separate "what the source says" from "what this deck should make the audience understand, decide, or do."
...
### 6.1.1 Upstream Methodology Pass

After the Eight Confirmations are approved and before drafting `design_spec.md`, complete a short planning pass:

1. **Source digest**: For file / URL / source-heavy projects, save `<project_path>/source_digest.md` with source inventory, core claims, important data points, named entities, assumptions, and omissions. For short chat-only briefs, place the same condensed summary in `design_spec.md §IX` instead of creating a separate file.
2. **Structured extraction**: Convert raw text into a table before page planning. Use columns such as `entity`, `dimension`, `metric`, `time_period`, `pain_point`, `evidence`, `recommendation`, and `source_ref`; adapt the schema to the deck. This prevents double-counting and exposes comparison patterns that are invisible in paragraph form.
3. **Density assessment**: Classify the material as light / normal / heavy. Estimate content load from page count, distinct entities, data tables, and decision points; recommend page count and decide what must be compressed, split, visualized, or cut. A good default is one core assertion plus 3-5 supporting facts per content slide. For comparison-heavy decks, use `entities × dimensions ÷ 3` as a rough content-page sizing check.
4. **Audience targeting**: State the primary reader and job-to-be-done. Executive decks lead with answer / hero metrics; technical decks preserve detail and caveats; client decks prioritize story and brand fit; public decks simplify language and avoid jargon.
5. **Narrative plan**: Lock the official `mode`, then describe the supporting argument framework in plain language. Pyramid, SCQA, STAR, AIDA, hero journey, comparison matrix, or a custom arc may be used as rationale, but the machine-readable `mode` remains the official value.
6. **Number hooks**: Pull reusable figures, rankings, deltas, years, thresholds, and ratios into one place so later slides can make claims with evidence instead of generic adjectives.
7. **Information translation plan**: Decide how source forms become slide forms: single critical number → hero metric; 3-5 items → card grid; 5-8 steps → timeline / flow; 9+ items → categorization or split; comparison data → table + color coding; process / evolution → timeline; conceptual relationships → diagram.
8. **Title track and chrome**: Draft the title sequence so it can tell the story by itself. Also define reusable page furniture (footer, page number, accent line, section marker, title treatment, insight strip) that Executor should mirror across pages.
9. **Focal hierarchy planning**: For each §IX page, commit to three tiers of information weight and record them in `spec_lock.md ## page_focal`.
   - **L1 (assertion)** — the single line a reader should remember if they only glance at the page. ≤18 CJK / ≤36 Latin chars. One per page. L1 is a one-line distillation of the page's `Core message`, not its `title`. Prefer assertions that can be agreed or disagreed with ("增长来自存量复购") over noun-phrase topics ("行业概览"). Anchor pages (cover / chapter / TOC) may use a short label.
   - **L2 (supporting)** — 2–4 items that make L1 credible (key metrics, named sub-themes, ranked drivers). Each ≤12 CJK / ≤24 Latin. The number of parallel content blocks Executor renders on the page MUST equal `len(L2)` — if you need 5 cards, either revise L1 to a tier higher or split the page.
   - **L3 (context)** — 0–3 items: source, date, scope, caveat, footnote. Each ≤10 CJK / ≤20 Latin. May be empty.
   - **Anti-patterns (planner-side)**: equal-weight 4-up card grids without L1; "every row is the same size" comparison tables; fake-cards-for-everything where each line of body text gets its own rounded container; multi-column layouts with identical visual emphasis across columns; noun-phrase topic labels for non-anchor pages.
   - **Output grammar** (written verbatim into `spec_lock.md`): `P<NN>: L1="<headline>" | L2=[item, item, ...] | L3=[item, item, ...]`. Validator: `scripts/validate_focal_hierarchy.py <project_path>` — must exit 0 before Executor begins Step 6.
10. **Palette role mapping**: After §III color decisions are made, map every HEX explicitly to a palette role and record them in `spec_lock.md ## palette_roles`.
    - **Required roles (must appear, validator blocks Step 6 if any missing)**: `bg_canvas`, `text_primary`, `text_secondary`, `border_subtle`, `accent_primary`. These are the minimum vocabulary any deck must speak — without them Executor has no canonical way to refer to the page surface, body text, or the brand accent.
    - **Optional roles (declare if used)**: surfaces (`surface_subtle`, `surface_emphasis`), text (`text_muted`), borders (`border_emphasis`), accents (`accent_subtle`, `accent_emphasis`), states (`state_success`, `state_warning`, `state_danger`, `state_info`). Add only roles the deck actually consumes — declaring unused roles dilutes the contract.
    - **One role, one HEX**: a HEX may serve only one role. If the same HEX would naturally cover two roles, pick the dominant role and reuse it semantically — do not declare it twice under different names.
    - **Relationship to `## colors`**: `## colors` stays as the rough working palette (Strategist's input panel + `update_spec.py` color-rewrite target). `## palette_roles` is the downstream truth that Executor enforces — every SVG fill / stroke / stop-color HEX MUST appear in `## palette_roles`. The two sections may differ in granularity (colors lists `primary` + `accent`; palette_roles can split `accent_primary` + `accent_subtle`), but every HEX in `## colors` should map onto at least one role.
    - **Anti-patterns (planner-side)**: declaring `accent_primary` and `accent_subtle` to the same HEX as a hedge ("we might split it later"); inventing decorative roles (`gradient_1`, `chart_blue`) instead of using the semantic vocabulary; leaving `state_*` roles undeclared and letting Executor improvise red/green for success/danger.
    - **Output grammar** (written verbatim into `spec_lock.md`): `- <role>: <HEX>` rows under `## palette_roles`. Validator: `scripts/validate_visual_consistency.py <project_path> --stage=lock-only` — must exit 0 before Executor begins Step 6.
11. **Page group planning**: For each §IX page, declare semantic slot groups in `spec_lock.md ## page_groups` so the Executor knows which `page_focal` items render as the hero, which as the supporting grid, and which as chrome.
    - **Group naming**: each group represents one visual register on the page. Recommended vocabulary: `hero` (the L1 assertion + any L2 promoted to equal weight), `grid` / `matrix` / `cards` (the L2 supporting items rendered as parallel blocks), `chrome` (page-level furniture: footer, page number, accent line — usually empty `[]` when chrome is supplied by the template), `footnote` / `source` (L3 context items). Use lowercase, `[a-z0-9_]+` only.
    - **Slot references**: each slot is `L1`, `L2_<N>`, or `L3_<N>` (N is 1-based index into the corresponding `page_focal` list). `L2_1` references the first L2 item, `L2_3` the third. Slots cannot reference items that do not exist in `page_focal.P<NN>`.
    - **Coverage rule**: the union of all slots across all groups on a page MUST cover every L1 + L2 + L3 item from `page_focal.P<NN>` exactly once. No item may appear twice; no item may be omitted. If a group is empty (`chrome=[]`), it still counts toward the group count but contributes no slots.
    - **Group count**: 2–4 groups per page is the healthy band. Validator emits a warning at 1 group (visually monolithic — likely missing a chrome / footnote slot) or ≥5 groups (over-fragmented — likely treating each L2 item as its own group, defeating the purpose).
    - **Visual implication**: within a group, all slots will render as **visually parallel** elements (same font ramp tier, same shape vocabulary, same palette role for the dominant fill, aligned to the same grid). Across groups, the visual treatment MUST differ — `hero` text larger than `grid` text; `grid` cards larger than `footnote` text; `chrome` distinguishable from `grid` (different rx, different fill role, or pulled to the page edge).
    - **Anti-patterns (planner-side)**: putting L3 (`footnote` / `source`) and L2 (`grid`) into the same group ("everything is cards"); declaring only one group (no semantic separation); naming groups by their content (`risk_a`, `risk_b`) instead of by their visual register (`grid`); splitting a single L2 list across two named groups for no narrative reason.
    - **Output grammar** (written verbatim into `spec_lock.md`): `P<NN>: <group_name>=[<slot>, <slot>, ...] | <group_name>=[<slot>, ...] | ...`. Validator: same `validate_visual_consistency.py --stage=lock-only` invocation as step 10.

This pass is not a user confirmation and must not slow the pipeline. Its purpose is to keep the deck from becoming a page-by-page source summary.
...
| IX. Content Outline | Grouped by chapter; each page includes layout, title, core message (the page's one idea), purpose, content blocks (in the selected phrasing mode), visualization / image intent, so-what, visualization type (if applicable) |
...
4. **Generate execution lock**: read `templates/spec_lock_reference.md` and produce `projects/<project_name>.../spec_lock.md` — a distilled, machine-readable short form of the color / typography / icon / image / **page_rhythm** / **page_layouts** / **page_charts** decisions above. This file is what the Executor re-reads before every page (see [executor-base.md](executor-base.md) §2.1). The values in `spec_lock.md` MUST exactly match the decisions recorded in `design_spec.md`; if they ever diverge, `spec_lock.md` wins and `design_spec.md` should be treated as historical narrative.
   ...
   - **page_focal is mandatory**: For every `P<NN>` listed in §IX, write one row under `## page_focal` using the grammar `P<NN>: L1="<headline>" | L2=[item, item, ...] | L3=[item, item, ...]` (tier sizes: L1=1, L2=2–4 items, L3=0–3 items). See §6.1.1 step 9 for tier semantics and length limits. Missing or malformed rows fail `scripts/validate_focal_hierarchy.py` and block Step 6 — Executor cannot proceed until Strategist returns and fixes the lock.
   - **page_tables (write only for table pages matching a tables catalog preset)**: For each page in §VII whose visualization is a designed table that adapts a `templates/tables/<name>.svg` preset, add `P<NN>: <table_name>`. Pages with a chart-side table (`basic_table` / `consulting_table` / `feature_matrix_table` in `templates/charts/`) MUST NOT appear here. **Mutual exclusion (HARD)**: a page MUST NOT appear in both `page_charts` and `page_tables` — the validator rejects the lock. If the deck has no preset-driven table pages, omit the section.
   - **palette_roles is mandatory**: Declare every HEX used in the deck as a named semantic role under `## palette_roles`. Required roles (`bg_canvas`, `text_primary`, `text_secondary`, `border_subtle`, `accent_primary`) must all appear — missing any one blocks Step 6. Optional roles (`surface_*`, `text_muted`, `border_emphasis`, `accent_subtle`, `state_*`) are declared only when consumed. See §6.1.1 step 10 for the full vocabulary and anti-patterns. Every SVG fill / stroke / stop-color HEX must later resolve to one of these declared values — Executor cannot introduce new HEXes mid-flight; if a page genuinely needs an undeclared color, Strategist must return and extend palette_roles.
   - **page_groups is mandatory**: For every `P<NN>` listed in §IX, write one row under `## page_groups` using the grammar `P<NN>: <group_name>=[<slot>, ...] | <group_name>=[<slot>, ...] | ...`. Each slot is `L1` / `L2_<N>` / `L3_<N>` referencing `page_focal.P<NN>`; the union of slots must cover every L1+L2+L3 item exactly once. See §6.1.1 step 11 for tier semantics and recommended group vocabulary (`hero` / `grid` / `chrome` / `footnote` / `source`). When `page_focal.L2` changes (item count, ordering), `page_groups` MUST be updated in lockstep — otherwise the slot references go stale and the validator rejects the lock.

**Per-page planning contract**: Each §IX page must remain executable after context compression. At minimum it needs title, layout, `Core message`, content blocks, and any visualization / image references. Add `Purpose` and `So What` whenever the page's role in the argument is not self-evident. If you cannot state the so-what, merge, cut, or rewrite the page before handing it to Executor.

**Outline self-test before handoff**:
- Title-track test: reading only page titles should reconstruct the deck's logic.
- Deletable-slide test: if removing a page does not break the argument, merge or cut it.
- Now-what test: the final section must resolve into recommendations, decisions, next actions, or an explicit conclusion.
- Focal-tier test: every content page must answer "what's the one line you'd remember" — if the page has 5+ parallel supporting items without a clear L1 assertion, restructure or split before handing to Executor.
- Visual-consistency test: every page can be explained with 2-3 semantic groups (hero / body / chrome). If you cannot name the groups, either L2 is too scattered or the page is trying to carry too many ideas — restructure before handing to Executor.
