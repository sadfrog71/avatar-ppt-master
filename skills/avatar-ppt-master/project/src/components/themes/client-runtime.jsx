...
function normalizeExternalValues(entry, defaults, values) {
  ...
  if (error) {
    console.warn(`[avatar-ppt-master] dropped invalid prop(s) for "${entry.key}" instead of crashing the render`, error);
  }
...
function renderRuntimeThemeSlide(slide, values = {}, options = {}) {
  ...
  try {
    ...
    console.error(`[avatar-ppt-master] runtime render failed for "${root.dataset.pageKey || entry.key}"`, error);
    return false;
  }
}
