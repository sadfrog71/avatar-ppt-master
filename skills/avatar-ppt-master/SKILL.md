...
description: 面向内部汇报、方案提案、项目复盘、培训和研究材料生成 PPT。先规划内容结构和页面角色，再组合预置视觉主题，输出可离线打开的 HTML 演示，并支持导出 PDF 和可编辑 PPTX。
---

# Avatar PPT Master

Avatar PPT Master 是面向内部同事使用的 PPT 生成 Skill。使用本 Skill 时，先把自然语言需求整理为内容规划和 JSON 计划，再调用本地生成器输出 `index.html` 与 `assets/`，并按需导出 PDF 或可编辑 PPTX。
