...
- **13 套视觉主题**：轻拟态、炫光紫绿、深浅代码、玻璃糖果、色谱图表、深色图谱、冷白调研、黑金实验、深蓝杂志、金色指数、高能增长、声波霓虹，以及新增的水务环境风
...
生成时可选择主题，也可随时让 Agent 整套换风格。README 不嵌入主题预览或演示图，避免引入来源不明的视觉素材。

### theme13 水务环境风（新增）

适合水务运营、环境管理、公用事业项目提案和数据复盘等需要「结论清楚、结构克制、可持续讨论」的汇报。主题提供 24 个高频管理版式，覆盖执行摘要、关键指标、趋势、对比、根因诊断、分层架构、协作治理、路线图、风险控制和优先级行动。

- **先结论后依据**：标题直接表达判断，数据、模型和对比页都配有读图结论。
- **内容不再单调**：根据内容选择 KPI、决策矩阵、诊断模型、流程、Roadmap 或行动卡片，而不是重复堆叠文本卡片。
- **保留原始视觉**：沿用蓝青配色、环形角标、卡片与页脚结构，不改变既有底版和页面效果。
...
- 🎨 **13 套视觉主题**：从轻拟态到声波霓虹，新增水务环境风，覆盖产品介绍、技术方案、数据报告、品牌故事、融资路演与管理型汇报
...
如果提供的是纯文字资料，Skill 会根据实际情况在合适的页面里预留图片位置，后续可再填入自有素材。
...
内置 9 种切换动画：液态形变、切入、横滑、行扫、缩放、竖条、混合、横切、画廊，也可以一键关闭。
