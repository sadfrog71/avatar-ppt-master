...
- **13 套视觉主题**：轻拟态、炫光紫绿、深浅代码、玻璃糖果、色谱图表、深色图谱、冷白调研、黑金实验、深蓝杂志、金色指数、高能增长、声波霓虹，以及新增的青蓝运营风
- **1044 个版式页面**：从几十种图表、SWOT / 波特五力 / 商业模式画布等分析模型，到架构图、时间线、图文画廊——做汇报真正会用到的页面都有现成版式
- **8712 个可调控件**：滑杆、开关、下拉，以及图片 / 图标选择器——拖一下就能增减页面模块或切换页面表达
...
生成时 Skill 会展示风格说明让你选，你也可以随时让 Agent 整套换掉。theme01–theme12 的预览均从各自版式库中挑选正文页，**全部由这个 Skill 真实渲染，非示意图**；theme13 的结构和适用规则见下方说明：
...
适合运营管理、项目提案、公共服务和数据复盘等需要「结论清楚、结构克制、可持续讨论」的企业汇报。主题提供 24 个高频管理版式，覆盖执行摘要、关键指标、趋势、对比、根因诊断、分层架构、协作治理、路线图、风险控制和优先级行动。

- **先结论后依据**：标题直接表达判断，数据、模型和对比页都配有读图结论。
- **内容不再单调**：根据内容选择 KPI、决策矩阵、诊断模型、流程、Roadmap 或行动卡片，而不是重复堆叠文本卡片。
- **统一且可控**：支持浅青蓝 / 暖白纸感两种整套背景基调；8 页以上的长 deck 可启用左侧章节导航。
- **风险有语义**：高 / 中 / 低风险使用红、黄、绿分级，并同时呈现责任人与控制动作；普通并列信息不会被误做成风险层级。
- **视觉克制**：单页只保留一个主视觉焦点，适合管理层快速阅读与会后继续编辑。
...
- 🎨 **13 套视觉主题**：从轻拟态到声波霓虹，新增青蓝运营风，覆盖产品介绍、技术方案、数据报告、品牌故事、融资路演与管理型汇报
- 🧩 **1044 个版式页面**：每套主题独立的页面结构和视觉语言，20 种页面角色（封面、目录、指标、趋势、对比、流程、风险、结尾……）
...
- 🔄 **一键换风格**：同一份内容在 13 套主题间实时换肤，还有 9 种翻页切换动画可选
...
2. **选风格** — 没指定风格时，Skill 会展示 13 套风格说明让你选，不会替你决定；同时确认是否需要图片 / 视频
...
写这份 README 时，13 套主题共提供 1044 个版式。做汇报真正会用到的页面，基本都能找到现成版式：
...
├── project/              ← 内置生成器（React 渲染、13 套主题、导出引擎）
│   ├── layout-manifest.json  ← 1044 个版式的字段契约清单
│   ├── scripts/              ← 渲染 / 校验 / 预览 / 导出脚本
│   └── packages/html-deck-to-pptx  ← 可编辑 PPTX 导出引擎（MIT 开源）
├── references/
│   ├── options.md            ← 主题清单与生成选项
│   ├── layout-pool.md        ← 13 套主题的版式池
